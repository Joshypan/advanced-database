#!/usr/bin/env python3
"""Rebuild the Reveal.js deck from topic-01-slides.md."""
import html
import json
...
slides = []
notes = ['# Chapter 1 Speaker Notes\n']
for number, chunk in enumerate(re.split(r'\n---\n', (root / 'topic-01-slides.md').read_text()), 1):
...
<title>Chapter 1: Working with SQL in SQLite</title>
...
(root / 'topic-01-slides.html').write_text(page)
(root / 'topic-01-speaker-notes.md').write_text('\n'.join(notes))
